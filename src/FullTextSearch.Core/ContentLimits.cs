...
/// </summary>
/// <remarks>
/// <para><b>方針:</b> ファイルサイズが <see cref="IndexMaxFileBytesForExtract"/> 以下なら、
/// 抽出した本文を<b>全文</b>インデックス・検索（Sudachi）・プレビュー表示する。文字数での打ち切りは行わない。</para>
/// </remarks>
public static class ContentLimits
{
    /// <summary>ファイルサイズ上限（1MB）。本文抽出・インデックス・テキスト読込・プレビュー試行で共通。</summary>
    public const long IndexMaxFileBytesForExtract = 1L * 1024 * 1024;

    /// <summary>テキストファイル読込上限。<see cref="IndexMaxFileBytesForExtract"/> と同値。</summary>
    public const long MaxTextFileBytesToRead = IndexMaxFileBytesForExtract;

    /// <summary>スキップ理由・UI 用のファイルサイズ上限表示。</summary>
    public static string GetIndexMaxFileBytesDisplayLabel() => "1MB";

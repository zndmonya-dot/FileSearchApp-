...
function Ensure-Dictionary {
    ...
    Invoke-WebRequest -Uri $dictUrl -OutFile $dictZip -UseBasicParsing
    Expand-Archive -Path $dictZip -DestinationPath $extractDir -Force
    $dic = Get-ChildItem -Path $extractDir -Recurse -Filter "system_core.dic" | Select-Object -First 1
...
function Update-DictionaryZip {
    ...
    Compress-Archive -Path $dictPath -DestinationPath $zipPath -CompressionLevel Optimal
}

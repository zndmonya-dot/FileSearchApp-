...
全体像は **「署名付き MSIX を作る（配布者）」→「各 PC で .cer を信頼してから .msix を入れる（利用者）」** です。

#### 配布者（ビルドする PC）で行うこと

| 順 | 作業 | 内容 |
|----|------|------|
| ① | **署名用証明書（秘密鍵）をストアに用意する** | **PFX** がある場合は「個人」ストアへインポートする。無ければ `.\scripts\create-cert-for-msix.ps1` でストアに直接作成する。 |
| ② | **拇印を csproj に書く** | `src\FileSearch.Blazor\FileSearch.Blazor.csproj` の `PackageCertificateThumbprint` を、①の証明書の Thumbprint に合わせる。これが「どの証明書で署名するか」の指定になる。 |
| ③ | **`dotnet publish` で MSIX を生成する** | [ビルドとMSIX作成.md](ビルドとMSIX作成.md) のコマンド。**パッケージへの署名はこの publish の処理の中で行われる。** |
| ④ | **配布用の .cer を用意する** | 同じ発行元の公開鍵だけのファイル。`create-cert-for-msix.ps1` 実行時に `全文検索システム_配布用.cer` が出力される、または `export-cert-pfx-cer.ps1` で .pfx と一緒に .cer を出力。 |
| ⑤ | **配布物をまとめる** | **.msix** + **.cer** + **インストール手順.txt** を同じフォルダ（または ZIP）にして渡す。 |

> **補足（「MSIX のあとに PFX を紐づける」について）**  
> MSIX に署名が付くのは **`dotnet publish` を実行したとき**です。先にストアに証明書（PFX 由来でもよい）を入れ、拇印を csproj に書いたうえで publish します。**生成済み MSIX に後から PFX を別手順でくっつける**のではなく、**publish 時点で署名済みの MSIX が出力される**、という理解で問題ありません。

#### 各クライアント PC（利用者）で行うこと

| 順 | 作業 | 内容 |
|----|------|------|
| ⑥ | **.cer を信頼する** | 配布された **.cer** を「信頼されたルート証明機関」にインストールする（初回のみ）。これで publish 時に付いた署名を PC が信頼できる。 |
| ⑦ | **.msix をインストールする** | ダブルクリック等でインストール。複数台のクライアントでは、それぞれ **⑥⑦** を行う（同じ .cer と .msix を使い回せる）。 |

詳細コマンドは下記 **A. 証明書**・**B. MSIX**、利用者向けはこのドキュメント冒頭の「利用者向け」を参照。

---

### 証明書ファイル（PFX / CER）とストアの関係

| 種類 | 内容 | 主な用途 |
|------|------|----------|
| **Windows 証明書ストア**（`Cert:\CurrentUser\My`） | 作成直後はここに秘密鍵付きで入る | `dotnet publish` が MSIX に署名するとき、**csproj の拇印（Thumbprint）** で参照する |
| **.pfx** | 秘密鍵付き（パスワードで保護可能） | **バックアップ**、別 PC への移動、チームで同じ証明書を使うときにインポート |
| **.cer** | 公開鍵のみ（秘密鍵なし） | **利用者に配布**し、PC の「信頼されたルート証明機関」に入れてもらう（MSIX を信頼させる） |

**ゼロからの流れ（おすすめ）**は次のとおりです。

1. `.\scripts\create-cert-for-msix.ps1` を実行する  
   → ストアに証明書が作成され、`installers\社内配布\全文検索システム_配布用.cer` が出力される。
2. 表示された **Thumbprint** を `FileSearch.Blazor.csproj` の `PackageCertificateThumbprint` に書く（下記 **A. パターン 1**）。
3. （任意）**PFX ファイルとしても保存**したい場合（バックアップ・別ビルド PC 用）は、同じ拇印で次を実行する。  
   ```powershell
   .\scripts\export-cert-pfx-cer.ps1 -Thumbprint "<ここに拇印>"
   ```  
   → `installers\社内配布\全文検索システム.pfx` と `全文検索システム_配布用.cer` が出力される。**PFX のパスワードは安全に管理すること**（リポジトリに書かない）。
4. **ビルド PC で MSIX を試しインストールする**ときは、利用者と同様に .cer をルートに追加するか、下記 A. パターン 1 の手順 5 の PowerShell でルートに追加する。
5. `dotnet publish` で MSIX を生成する（[ビルドとMSIX作成.md](ビルドとMSIX作成.md) のコマンド）。

**別 PC でビルドする場合**: その PC の「個人」ストアに**同じ秘密鍵**が必要です。`全文検索システム.pfx` をコピーし、パスワードを入力してインポート（例: 「PFX の右クリック」や PowerShell の `Import-PfxCertificate`）。インポート後の拇印が変わる場合は、`csproj` の `PackageCertificateThumbprint` をその拇印に合わせる。

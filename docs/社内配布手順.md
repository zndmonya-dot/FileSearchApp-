...
開発 PC では Sudachi を再ビルドしない。**ビルド担当 PC** が DLL を作成し、**社内 SVN**（または開発用リポジトリ）にコミットする。

1. Rust（stable）と .NET 8 SDK を入れた PC で:

```powershell
pwsh -File scripts/build-sudachi-native.ps1
```

2. `tools\sudachi\` を SVN に追加・コミットする（初回は `svn add` が必要なファイルあり）。  
3. 開発者は `svn update` で `tools\sudachi\` を取得してから `dotnet build` する。

DLL 更新は Sudachi バージョン変更時など **年に数回以下** でよい。詳細は [tools/sudachi/README.md](../tools/sudachi/README.md)。

### 3.1 作業一覧

| 順 | 内容 |
|----|------|
| 0 | （初回のみ）Sudachi 同梱物をビルドし SVN にコミット（上記 3.0） |
...
3. **`installers\社内配布\Panoleon_配布用.cer`** が出力される → 配布用の **.cer** として使う。  
4. （任意）PFX もファイルで残す場合:
...
→ `installers\社内配布\Panoleon.pfx` と .cer。PFX のパスワードは安全に管理する。
...
```powershell
Import-Certificate -FilePath "C:\配布物のパス\Panoleon_配布用.cer" -CertStoreLocation "Cert:\LocalMachine\Root"
```
...
- 配布者: `scripts\build-dist.bat`（または `build-dist.ps1`）→ `installers\dist\Panoleon_win-x64.zip`。実行時は `publish` の前に `check-webview-strings.ps1` が走り、`index.html` と `UserMessages` の整合を確認する。

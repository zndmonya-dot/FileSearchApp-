...
| **PreviewService.cs** | PreviewMaxChars | 50_000 | プレビュー上限（同上）。 |
| **PreviewService.cs** | PreviewMaxLinesForHighlight | 500 | ハイライト用バッチの最大行数（JS 往復を抑えるため 500 行単位）。 |
| **PreviewService.cs** | HighlightBatchSize | 50 | 1 バッチあたりのハイライト行数。 |
| **PreviewService.cs** | DiagnosticHtmlSampleLength | 400 | 診断用 HTML サンプル長。 |
...
| ExtensionLanguageMessage | 拡張子を入力してください（例: .cs） | 拡張子→言語で拡張子が空のとき |
| ExtensionLanguageMessage | 言語名を入力してください（例: csharp） | 拡張子→言語で言語名が空のとき |

...
| indexSkipCount / indexSkipLogPath | 件数とログパスを別表示（フッターで折り返し） | インデックス完了後、`SearchSidebar` のスキップ表示 |
...
| PreviewService | [プレビュー不可] / [キャンセル] / [エラー] … | 抽出器なし・キャンセル・抽出失敗 |
| Home.razor.cs | 読み込みをキャンセルしました / [エラー] … | プレビュー読み込みキャンセル・例外 |
...
| FolderMessage | フォルダの選択に失敗しました: … | 参照ダイアログ失敗時 |

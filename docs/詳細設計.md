...
- **起動・ウィンドウタイトル**: App.xaml.cs で `CreateWindow` をオーバーライドし、`Window.Title = "全文検索システム"` を設定している。MAUI Windows では ContentPage の Title はタイトルバーに反映されないため、ここで設定する。MainPage.xaml の ContentPage にも Title を指定しているが、タイトルバー表示は CreateWindow に依存する。
- **プレビュー**: FilePreviewView は行リスト表示のみ（html モードは廃止）。ファイル選択時は SchedulePreviewLoad で **200ms**（`PreviewDebounceMs`）デバウンス後に LoadPreview を実行。
- **インデックス構築のキャンセル**: RunIndexUpdateAsync で CancellationTokenSource を保持し、RebuildIndexAsync / UpdateIndexAsync に token を渡す。フッターに「キャンセル」ボタンを表示し、押下で Cancel を呼ぶ。OperationCanceledException 時は進捗テキストを「キャンセルしました」に更新する。進捗表示には IndexProgress.CurrentFile を含め、処理中のファイル名を表示する。
- **スキップ通知**: インデックス完了後、`IndexService.LastSkippedFiles` にスキップ件数があれば `indexSkipCount` と `indexSkipLogPath` を設定し、SearchSidebar のフッターに警告色で 2 行表示する（ログパスは折り返し、`title` に全文）。
...
| LastSkippedLogPath | 直近のスキップログファイルのパス（スキップがなければ null） |
...
- **スキップログ出力**: 再構築・差分更新の完了後、`WriteSkippedLog` でインデックスフォルダ内に `skipped_files.log` を出力。UI には `LastSkippedFiles` / `LastSkippedLogPath` 経由で件数とパスを表示する。
...
- PreviewService（Blazor）は、抽出器でテキストを取得し、常に Mode = \"text\"・行リスト（Lines）で返す。Office/PDF もテキスト抽出して行リストで表示し、コードであっても構文色分けは行わない。検索語のみを `<mark>` タグで強調表示する。UI（Home.razor.cs）ではファイル選択時に **200ms** デバウンス（`PreviewDebounceMs`）をかけて LoadPreview を呼び、連続クリック時の無駄な抽出を抑えている。

...
| Settings | 設定の永続化・動作モード | AppSettingsService, AppModeService |
...
| RebuildIndexAsync(folders, progress, options, cancellationToken) | 再構築（全削除＋全件追加）。キャンセル時はロールバックで実行前の状態に戻る |
| UpdateIndexAsync(folders, progress, options, cancellationToken) | 差分更新（追加・更新・削除のみ反映）。検索対象フォルダ（TargetFolders）から外れたフォルダ配下にあるインデックス済みファイルも削除対象に含む。キャンセル時はロールバックで実行前の状態に戻る |
| GetStats() | 統計情報取得 |
...
- **サイズ超過スキップ**: `ContentLimits.IndexMaxFileBytesForExtract`（10MB）を超えるファイルは本文抽出・登録を行わずスキップ。
- **本文**: 10MB 以下のファイルは抽出全文を Lucene 保存・Sudachi トークン化・バイグラム・プレビューに使用（文字数打ち切りなし）。
...
- **利用ライブラリ補足**: OfficeExtractor は `DocumentFormat.OpenXml`、PdfExtractor は `PdfPig`、TextFileExtractor の文字コード判定は `UtfUnknown` を利用する。

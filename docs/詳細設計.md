...
- **サイズ超過スキップ**: 10MB（`ContentLimits.IndexMaxFileBytesForExtract`、プレビュー・テキスト読み込み上限の 10MB と同値）を超えるファイルは本文抽出せずスキップ（null 返却）。スキップされたファイルは `_skippedFiles` に記録される。
- **コンテンツ打ち切り**: 抽出テキストが 100,000 文字（`ContentLimits.ExtractMaxChars`）を超える場合、抽出器（OfficeExtractor / PdfExtractor）が打ち切る。さらに Lucene 格納時にも `ContentLimits.IndexMaxContentChars`（100,000 文字）で打ち切り。

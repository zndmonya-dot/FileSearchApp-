...
- **プレビュー**: FilePreviewView は行リスト表示のみ（html モードは廃止）。ファイル選択時は SchedulePreviewLoad で 300ms デバウンス後に LoadPreview を実行。
...
- PreviewService（Blazor）は、抽出器でテキストを取得し、常に Mode = \"text\"・行リスト（Lines）で返す。Office/PDF もテキスト抽出して行リストで表示し、コードであっても構文色分けは行わない。検索語のみを `<mark>` タグで強調表示する。UI（Home.razor.cs）ではファイル選択時に 300ms デバウンスをかけて LoadPreview を呼び、連続クリック時の無駄な抽出を抑えている。

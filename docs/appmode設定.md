...
**中身**（検索フォルダ・インデックス場所・サイズ上限）は管理者がアプリから更新します。
...
| `indexMaxFileBytes` | このサイズ（バイト）超のファイルはスキップ。`0` = 無制限、省略 = 1MB |
...
管理者は設定画面の **「スキップするファイルサイズ上限（MB）」** からも変更でき、保存時に `shared.json` へ書き込まれます（`0` = 無制限）。
